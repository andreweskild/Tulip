...
void TulipControlsPlugin::initializeEngine(QQmlEngine *engine, const char *uri)
{
    Q_ASSERT(QLatin1String(uri) == QLatin1String("Tulip.Controls"));

    // For system icons
    engine->addImageProvider(QLatin1String("tulipicontheme"), new IconThemeImageProvider());
}

void TulipControlsPlugin::registerTypes(const char *uri)
{
    Q_ASSERT(QLatin1String(uri) == QLatin1String("Tulip.Controls"));
