...
Project {
    ...
    Product {
        ...
        Rule {
            ...
            prepare: {
                ...
                cmd.prefix = FileInfo.joinPaths(product.moduleProperty("qbs", "installPrefix"),
                                                product.moduleProperty("tulipdeployment", "includeDir"));
                cmd.content = project.resolvedProperties;
                cmd.sourceCode = function() {
                    content.Depends = (content.Depends || []).concat([{ name: TulipUtils.quote("cpp") }]);
                    content["cpp.includePaths"] = TulipUtils.includesForModule(project.moduleName, prefix, project.version).map(TulipUtils.quote);
                    content["cpp.dynamicLibraries"] = [TulipUtils.quote(project.targetName)];

                    if (project.rpath)
                        content["cpp.rpaths"] = product.moduleProperty("cpp", "rpaths").map(TulipUtils.quote);

                    var cxxStandardLibrary = product.moduleProperty("cpp", "cxxStandardLibrary");
                    if (cxxStandardLibrary)
                        content["cpp.cxxStandardLibrary"] = TulipUtils.quote(cxxStandardLibrary);

                    var cxxLanguageVersion = product.moduleProperty("cpp", "cxxLanguageVersion");
                    if (cxxLanguageVersion)
                        content["cpp.cxxLanguageVersion"] = TulipUtils.quote(cxxLanguageVersion);

                    var outputFile = new TextFile(output.filePath, TextFile.WriteOnly);
                    outputFile.writeLine("import qbs");
                    outputFile.writeLine("import TulipUtils");
                    outputFile.writeLine("");
                    outputFile.write(TulipUtils.jsToQml({ Module: content }));
                    outputFile.close();
                };
                return [cmd];
            }
        ...
        Group {
            qbs.install: true
            qbs.installDir: FileInfo.joinPaths(tulipdeployment.qbsModulesDir, product.moduleName)
    ...
    Product {
        ...
        Rule {
            ...
            prepare: {
                ...
                cmd.prefix = product.moduleProperty("tulipdeployment", "prefix");
                cmd.installPrefix = FileInfo.joinPaths(product.moduleProperty("qbs", "installPrefix"), cmd.prefix);
                cmd.libDir = product.moduleProperty("tulipdeployment", "libDir").replace(cmd.prefix + "/", "");
                cmd.includeDir = product.moduleProperty("tulipdeployment", "includeDir").replace(cmd.prefix + "/", "");
                cmd.includePaths = TulipUtils.includesForModule(project.moduleName,
                                                               product.moduleProperty("tulipdeployment", "includeDir"),
                                                               project.version);
                cmd.sourceCode = function() {
        ...
        Group {
            qbs.install: true
            qbs.installDir: FileInfo.joinPaths(tulipdeployment.libDir, "pkgconfig")
    ...
    Product {
        ...
        Rule {
            ...
            prepare: {
                ...
                cmd.prefix = FileInfo.joinPaths(product.moduleProperty("qbs", "installPrefix"),
                                                product.moduleProperty("tulipdeployment", "prefix"));
                cmd.libDir = FileInfo.joinPaths(product.moduleProperty("qbs", "installPrefix"),
                                                product.moduleProperty("tulipdeployment", "libDir"));
                cmd.includeDir = FileInfo.joinPaths(product.moduleProperty("qbs", "installPrefix"),
                                                    product.moduleProperty("tulipdeployment", "includeDir"));
                cmd.includePaths = TulipUtils.includesForModule(project.moduleName, cmd.includeDir, project.version);
                cmd.sourceCode = function() {
                    var findDeps = "";
        ...
        Group {
            qbs.install: true
            qbs.installDir: FileInfo.joinPaths(tulipdeployment.libDir, "cmake", project.moduleName)
