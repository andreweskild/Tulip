...
Product {
    property stringList includePaths: TulipUtils.includesForModule(
                                          sync.module,
    ...
    Group {
        ...
        qbs.installDir: FileInfo.joinPaths(tulipdeployment.includeDir, sync.module)
        fileTagsFilter: ["hpp_public", "hpp_forwarding", "hpp_module", "hpp_depends"]
    }

    Group {
        qbs.install: product.install
        qbs.installDir: FileInfo.joinPaths(tulipdeployment.includeDir, sync.module, project.version, sync.module, "private")
